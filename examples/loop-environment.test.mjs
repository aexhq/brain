...
test("a turn outside Brain calls back through its token and returns what the loop kept", async () => {
  ...
  const turned = await command({
    ...
    callback: { url: "http://brain.example/v1/sessions/ses_one/executions/3/call", token: "execution-token", methods: ["model", "emit"] },
  });
  ...
  assert.deepEqual(calls.map(({ url }) => url), [
    "http://brain.example/v1/sessions/ses_one/executions/3/call",
    "http://brain.example/v1/sessions/ses_one/executions/3/call",
  ]);
  assert.ok(calls.every(({ authorization }) => authorization === "Bearer execution-token"));
  assert.equal(calls[1].body.input.messages.length, 1);
