...
test("advances its cursor only after the queue acknowledges an event", async () => {
  let cursor = 0;
  let attempts = 0;
  const events = [{ event_id: "evt_1", sequence: 1, event_type: "turn_started", data: {} }];
  const brain = {
...
test("drains more than one finite journal page in a run", async () => {
  ...
  const brain = {
    ...
    async readEvents(_sessionId, after) {
      ...
      return { events: [{ event_id: `evt_${sequence}`, sequence, event_type: "test", data: {} }], next_cursor: sequence };
    },
  };
  const cursors = {
    async load() { return cursor; },
