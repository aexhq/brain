...
const lookup = tool({
  ...
  run: async ({ id }, ctx) => {
    ...
    return ctx.finish({ value: `${ctx.options.prefix}${id}` });
  },
});
...
and the input. The host validates input and each output and posts ordered updates.
`ctx` contains `options`, `sequence`, optional `deadline`, `signal`, `emit`, `emitResult`, and `finish`. Awaiting `ctx.emit(kind, data)`
waits until Brain has committed the extension event.

Return ends only the synchronous phase. The Tool may keep emitting after return; `finish(value?)`
commits completion and rejects further emissions. Use `return ctx.finish(value)` for an ordinary
Tool. A missing finish leaves it open under its original deadline and cancellation signal.
Background results survive normal turn completion, enter journal/history, and wake the Agentloop.
The loop decides whether to use them in model messages. No result is invented for an empty return
or finish. See [Tool lifecycle](/brain/docs/guides/write-a-tool#return-values-and-outcomes).
...
Brain validates canonical input and output schemas for every placement. Brain commits
results, return, and completion before the Agentloop sees them, and never
