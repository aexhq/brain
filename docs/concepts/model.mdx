...
The system prompt and the tools are set at create and are not pinned: the agent loop may replace
the prompt or offer a subset of the tools on any model call. The response format is the loop's,
per call.
