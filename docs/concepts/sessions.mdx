...
`*_started` record to the log. Before the loop sees a result, a `*_ended` record is written too,
naming the started record by its sequence; an effect that did not come back with a result gets a
`*_failed` record instead, which says whether the effect may have happened anyway. Every
lifecycle uses the same three words: `turn_started`, `turn_ended`, `turn_failed`;
`session_creation_started`, `session_creation_ended`, `session_creation_failed`. A record is
named by the session id and its sequence, and those are the only ids there are: a client-hosted
tool call is answered at the sequence of its `tool_call_started`, and an environment sees the
same pair on the wire.
...
turn ended comes back idle with its history readable, and one that was mid-turn comes back with
a `turn_failed` record whose code is `interrupted` and returns to idle. Whether the in-flight call reached the model or a
