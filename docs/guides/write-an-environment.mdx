...
invocations stay on the environment's own `run` handler (the legacy runtime path).
Environments only ever execute provisioned tools — a tool answered by an application
process never touches one; Brain routes those itself (see
[App tools](/brain/docs/guides/app-tools)).
