...
[runnable package example](https://github.com/aexhq/brain/tree/main/examples/packaged-tools)
contains a filesystem Tool, a normal `tsc` build, and two package exports. `brain.tools` in its
...
Consumers import the factory from the package and call `readText({ env: workspace })`. Its browser
...
Environment implementers can use `@aexhq/brain/runtime` and the `brain-tool-runtime` process bridge.
Install packages in the isolated runtime, then use its supported loader; do not import customer
packages into the Environment controller. The bridge supports events, intermediate results,
return-before-finish, model calls and explicit completion.
...
[document summarizer](https://github.com/aexhq/brain/blob/main/examples/tool-model.mjs) is a complete
...
reconstruction is needed. The [Python package example](https://github.com/aexhq/brain/tree/main/tests/fixtures/python-project)
exports metadata from Python and runs in a prepared Python Environment. Its JavaScript caller
is exercised in [the cross-language test](https://github.com/aexhq/brain/blob/main/examples/python-environment.test.mjs).
...
quickstart's Docker command. This requires a Brain server you control. On Aex, use a
[managed Python profile](https://github.com/aexhq/aex/blob/main/docs/environments.md) granted to your account.
Use a [Python environment service](/brain/docs/guides/write-an-environment#python-and-other-languages)
