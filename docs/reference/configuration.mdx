...
| `BRAIN_MAX_TURN_INPUT_BYTES` | `--max-turn-input-bytes` | `33554432` | Bytes one turn's input to an Agentloop may hold, transcript and events together |
| `BRAIN_MAX_TURN_OUTPUT_BYTES` | `--max-turn-output-bytes` | `33554432` | Bytes one turn's output from an Agentloop may hold |
...
| `BRAIN_MAX_CONCURRENT_TURNS` | `--max-concurrent-turns` | `8` | Agentloop turns one worker runs at once, and separately native Tool calls. Each holds a fresh Store while it runs, so guest memory is bounded by twice this count times the linear memory ceiling |
