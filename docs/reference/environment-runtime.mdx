...
description: Resource grants, runtime limits and worker configuration.
---

An Environment is anything that implements the Environment protocol. A session names each one
once; Tool placements and the Agentloop refer to those names; explicit lifecycle policy controls setup
...
Session creation sets automatic bindings up before reporting ready. Manual bindings wait for
an authorized setup operation. Brain journals each operation
