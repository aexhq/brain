...
The session's tool is a plain function in your own process: declare it once with `tool`,
...
`tool_call_started` record and surfaces on the session's event feed, your `execute` answers
it beside the `orders` object it closes over, and the SDK posts the outcome back under that
record's sequence — idempotently, so a retry is the same effect. The whole exchange
lands in the feed, tool call and result included.

Under the hood this is `hosting: "client"` on the tool's manifest. When the function
lives in a *different* process — a browser page your backend opened the session for, a
CLI on the user's laptop — declare the tool without `execute` and let that process join
the session with its share key and serve it: see
[App tools](/brain/docs/guides/app-tools). A tool that needs `exec`, `fs`, or a sandbox
runs provisioned in an environment instead: see
[Environments](/brain/docs/concepts/environment).
