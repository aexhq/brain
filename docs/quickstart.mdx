---
title: Quickstart
description: Run Brain and give a session a Tool from your own process.
---
...
const lookupOrder = tool({
  ...
  run: async ({ id }, ctx) => {
    ...
    return ctx.finish(orders[id] ?? { status: "unknown order" });
  },
});
...
and a [lazy Environment](https://github.com/aexhq/brain/blob/main/examples/lazy-environment.mjs).
...
and [Write an Environment](./guides/write-an-environment) for the host calls and the Environment protocol.
