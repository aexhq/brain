...
try {
  ...
  if (createHash("sha256").update(bundleBytes).digest("hex") !== request.seal.code_digest) {
  ...
  if (tool.name !== request.seal.name) {
    throw new TypeError("bundle Tool runtime does not match the sealed name");
  }
  ...
  for (const name of request.seal.required_env) {
