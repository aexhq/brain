...
/// What one loop worker may hold and spend. Brain ships a default for each and never
...
pub struct LoopLimits {
    ...
    #[arg(long, env = "BRAIN_MAX_PACKAGE_BYTES", default_value_t = LoopLimits::default().max_package_bytes)]
    pub max_package_bytes: usize,
    /// Bytes one turn's input to an Agentloop may hold, transcript and events together.
    #[arg(long, env = "BRAIN_MAX_TURN_INPUT_BYTES", default_value_t = LoopLimits::default().max_turn_input_bytes)]
    pub max_turn_input_bytes: usize,
    /// Bytes one turn's output from an Agentloop may hold.
    #[arg(long, env = "BRAIN_MAX_TURN_OUTPUT_BYTES", default_value_t = LoopLimits::default().max_turn_output_bytes)]
    pub max_turn_output_bytes: usize,
    /// Bytes of linear memory one Wasm instance may grow to.
    #[arg(long, env = "BRAIN_MAX_LINEAR_MEMORY_BYTES", default_value_t = LoopLimits::default().max_linear_memory_bytes)]
    ...
    #[arg(long, env = "BRAIN_MAX_FUEL", default_value_t = LoopLimits::default().max_fuel)]
    pub max_fuel: u64,
    /// Agentloop turns one worker runs at once, and separately native Tool calls. Each
    /// holds a fresh Store while it runs, so guest memory is bounded by twice this count
    /// times the linear memory ceiling.
    #[arg(long, env = "BRAIN_MAX_CONCURRENT_TURNS", default_value_t = LoopLimits::default().max_concurrent_turns)]
    ...
    #[arg(long, env = "BRAIN_MAX_CORE_INSTANCES", default_value_t = LoopLimits::default().max_core_instances)]
    pub max_core_instances: usize,
    /// Seconds one outbound HTTP request from a native Component may take.
    #[arg(long, env = "BRAIN_MAX_NATIVE_HTTP_SECS", default_value_t = LoopLimits::default().max_native_http_secs)]
...
impl Default for LoopLimits {
    fn default() -> Self {
        Self {
            ...
            max_turn_input_bytes: 32 * 1024 * 1024,
            max_turn_output_bytes: 32 * 1024 * 1024,
...
impl LoopLimits {
    ...
    pub fn args(&self) -> Vec<String> {
        vec![
            ...
            format!("--max-turn-input-bytes={}", self.max_turn_input_bytes),
            format!("--max-turn-output-bytes={}", self.max_turn_output_bytes),
            format!("--max-linear-memory-bytes={}", self.max_linear_memory_bytes),
            format!("--max-fuel={}", self.max_fuel),
            format!("--max-concurrent-turns={}", self.max_concurrent_turns),
...
mod tests {
    ...
    fn the_rendered_command_line_parses_back_to_the_same_limits() {
        let limits = LoopLimits {
            max_package_bytes: 1,
            max_turn_input_bytes: 2,
            max_turn_output_bytes: 3,
        ...
        let mut argv = vec![
            "brain-loop-worker".to_owned(),
            "/run/worker.sock".to_owned(),
        ];
    ...
    fn the_worker_defaults_match_the_supervisor_defaults() {
        let parsed = WorkerArgs::parse_from(["brain-loop-worker", "sock"]);
        assert_eq!(parsed.limits, LoopLimits::default());
    }
}
