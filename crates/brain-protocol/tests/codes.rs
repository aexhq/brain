...
fn the_published_catalogue_matches_the_crate() {
    let published: serde_json::Value =
        serde_json::from_str(include_str!("../generated/contract/session/v1/codes.json"))
            .expect("codes.json parses");
    assert_eq!(
        published,
        brain_protocol::codes::catalogue(),
        "generated/contract/session/v1/codes.json is out of step with brain_protocol::codes"
    );
}
