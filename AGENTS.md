...
- The Rust types in `crates/brain-protocol`, the `#[utoipa::path]` annotations in
  ...
  `generated/contract/` with `cargo run -p <crate> --bin <crate>-contract`, and the SDK's
  `src/generated` is rendered from those by `npm run gen`. The WIT under
  `crates/brain-env/wit/` is written by hand. Never edit a file under `generated/`; CI
...
- Real Linux brain-env, image, HTTP, journal recovery, remote-model, and remote-Environment gates run
